...
def compute_Xapp4_utility(kpis):
    return compute_utility_value(
        (kpis[3] + kpis[4]) / 2,
        (
            (MEAN_STD_KPIS[3][0] + MEAN_STD_KPIS[4][0]) / 2,
            (MEAN_STD_KPIS[3][1] + MEAN_STD_KPIS[4][1]) / 2,
        ),
    )
...
class ORANEnvironment2(gym.Env):
    def __init__(self, num_bins=10, max_steps=50):
        ...
        xapp_utility_fns = [
            compute_Xapp1_utility,
            compute_Xapp2_utility,
            compute_Xapp3_utility,
            compute_Xapp4_utility,
            compute_Xapp5_utility,
        ]
        ...
        xapp_directions = [direction[0], direction[1], direction[2], direction[3], direction[5]]
        ...
        # ----- Parameters -----
        self.params = [Param(ParamThresholds[i], i, setParamFns[i]) for i in range(8)]
        ...
        self.xapps = [
            XApp(
                threshold=xapp_thresholds[i],
                utility_fn=xapp_utility_fns[i],
                name=xapp_names[i],
                params=xapp_params[i],
                direction=xapp_directions[i],
                mean_std=xapp_mean_stds[i],
            )
            for i, _ in enumerate(xapp_names)
        ]
        ...
        self.action_dim = 3
        self.min_bin_length = int(
            np.min([param[1] - param[0] for param in self.paramThresholds]) // self.num_bins
        )
        # self.max_bin_length = int(np.max([param[1]-param[0] for param in self.paramThresholds]) // self.num_bins)
        self.bin_length_per_param = [
            int((param[1] - param[0]) // self.num_bins) for param in self.paramThresholds
        ]
    ...
    def reward(self, new_kpis, nw_kpis):
        ...
        satisfied_kpis = [
            kpi.value >= kpi.kpi_threshold if kpi.direction == 0 else kpi.value <= kpi.kpi_threshold
            for kpi in new_kpis
        ]
        reward = sum(satisfied_kpis) ** 2

        distance = [
            (kpi.compute_utility_value() - kpi.compute_utility_threshold())
            if kpi.direction == 0
    ...
    def _get_state(self):
        ...
        for i, val in enumerate(self.prev_params):
            ...
            normalized = (
                (val - low) / (high - low) if high != low else 1
            )  # avoid division by zero if thresholds are the same
            # normalized = val
            state[f"param{i}"] = np.array([normalized], dtype=np.float32)

        for kpi in self.kpis:
            val = kpi.compute_utility_value() + np.random.normal(
                0, 0.01
            )  # add small noise to utility value to make it more realistic
            # val = kpi.value
            name = kpi.name
