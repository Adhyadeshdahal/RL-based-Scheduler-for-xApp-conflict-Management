...
class ORANEnvironment(gym.Env):
    ...
    def __init__(self, num_bins=10, max_steps=50):
        ...
        self.params = [Param(self.paramThresholds[i], i, setParamFns[i]) for i in range(7)]
        ...
        self.min_bin_length = int(
            np.min([p[1] - p[0] for p in self.paramThresholds]) // self.num_bins
        )
        self.bin_length_per_param = [
            int((p[1] - p[0]) // self.num_bins) for p in self.paramThresholds
        ]
