...
class ModelBasedPolicy:
    ...
    def __init__(
        ...
        self.weights_per_xapps = (
            weights_per_xapps if weights_per_xapps is not None else [1.0] * len(self.xapps)
        )
    ...
    def _cem(self, initial_state: torch.Tensor) -> np.ndarray:
        ...
        action_bounds = torch.tensor(
            self.action_space, dtype=torch.float32, device=self.device
        )  # (action_dim,)
        ...
        for _ in range(self.n_iter):
            noise = torch.randn(
                self.n_candidate, self.n_horizon, self.action_dim, device=self.device
            )
            action_candidates = (
                (mean + std_dev * noise).round().long()
            )  # (n_candidate, n_horizon, action_dim)
            ...
            predicted_kpis = self._rollout(
                initial_state, action_candidates
            )  # (n_candidate, n_horizon, n_kpis)
        ...
        best_action = torch.stack(
            [best_action[i].clamp(0, self.action_space[i]) for i in range(self.action_dim)]
        )  # (action_dim,)
    ...
    def _rollout(self, initial_state: torch.Tensor, action_sequences: torch.Tensor) -> torch.Tensor:
        ...
        state = (
            initial_state.unsqueeze(0).expand(num_candidates, -1).clone().float()
        )  # (n_candidate, state_dim)
        predicted_kpis_list = []
        ...
        for step in range(self.n_horizon):
            ...
            for i in range(num_candidates):
                param_id, bin_id, index = (
                    action_batch[i, 0].item(),
                    action_batch[i, 1].item(),
                    action_batch[i, 2].item(),
                )
